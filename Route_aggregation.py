...
def token_calculator(route_object, user):
    tokens = 0
    user_rank = user.rank
...
def score_calculator(route_object, user): # This function will calculate rank change for the route
    import numpy as np
    last_eco_relative_routes = user.eco_relative_scores[-30:]
    last_eco_relative_routes.append(eco_relative(route_object))
    
    return(np.mean(100 - last_eco_relative_routes))


def presentable_data(route_object, user):
    ...
    score = score_calculator(route_object)   # This will be a function that calculates rank change for the 
